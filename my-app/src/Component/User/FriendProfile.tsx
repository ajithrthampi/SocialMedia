...
const FriendProfile = () => {
  ...
  // const isFriendPostModal = useSelector((state:any) => state.userDetails.value.friendShowModal)
  // console.log("Post Modal",isFriendPostModal);
  console.log("Helloo,,,,,.,", isprofileDetails);
  ...
  useEffect(() => {
    try {
      ...
      axiosinstance.get("/followingcount/" + reduxState?.userId?._id, {
        headers: {
  ...
  const viewPost = () => {
    ...
    axiosinstance.get("/viewprofilepost/" + reduxState?.userId?._id, {
      headers: {
        "x-access-token": localStorage.getItem("token"),
      },
    }).then((response) => {
            ...
            <>
              <div className=' box-content h-auto  bg-[2A2A2A] rounded-3xl px-4 py-3'>
                <div className=' grid grid-cols-3'>
                  <div>
                    {reduxState ? <img className="p-1 mx-auto  w-28 justify-content-center h-28 rounded-full ring-2 ring-gray-300 dark:ring-gray-500" src={`images/${reduxState?.userId?.Images}`} alt="Bordered avatar" />
                ...
                <div className=''>
                  <div className='sm:px-10 px-'>
                    <div className='text-white pt-6'>
                      <h1>{reduxState?.userId?.name}</h1>
                      <h3 className='text-sm text-[#737373] pt-1 py-3 '>{reduxState?.userId?.username}</h3>
                    </div>

                    {/* BIO */}
                    <div className='text-white font-semibold'>{reduxState?.userId?.bio}</div>
            ...
            <div className='max-h-screen overflow-y-scroll scrollbar-none'>
              <div className='flex  lg:p-10 p-5 gap-12 lg:pl-40 pl-32'>
                ...
                <div className='  rounded-full  cursor-pointer'>
                  <img className='rounded-full w-36 h-36  object-cover overflow-hidden' src={`images/${reduxState?.userId?.Images}`} alt="" />
                </div>
                <div>
                  <>
                    <div className='xl:flex gap-12   xl:space-y-0 space-y-5 text-lg'>
                      <div>{reduxState?.userId?.username}</div>
                    ...
                    <div className='xl:mt-5 mt-3    '>
                      <h1>{reduxState?.userId?.name}</h1>
                      <h4 className='text-[#c9c6c6b1]'>{reduxState?.userId?.bio}</h4>
                    </div>
                  </>
                </div>
              </div>
